...
import { HazardIcon, DiseaseIcon, PrecautionIcon, ChevronDownIcon, SummaryIcon, MapPinIcon, BuildingOfficeIcon, DirectionsIcon, MapIcon, RouteIcon } from './icons';
    ...
    <div className="p-4 sm:p-6 space-y-6 animate-fade-in">
        ...
        <div className="flex gap-2">
            <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold transition-all ${showRouteAnalysis ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-blue-600 border-2 border-blue-100 hover:border-blue-200'}`}
            >
                <DirectionsIcon className="w-5 h-5" />
                {showRouteAnalysis ? 'View Health Risks' : 'Analyze Medicine Routes'}
            </button>
        </div>

        {showRouteAnalysis ? (
            <MedicineRouteAnalysis defaultTo={result.locationName} />
        ) : (
